...
class GeneCategories(object):
    ...
    def colorize_shortest_path(self, from_gene, to_gene, color="red", directed=True):
        ...
        if to_gene not in back_path:
            raise KeyError("no path found from %s to %s" % (repr(from_gene), repr(to_gene)))
        gene = to_gene
        path = set()
        ...
        for (gene1, gene2) in path:
            ...
            #N.styling_overrides[svg_name] = color
            N.styling_overrides.set_overrides(svg_name, {"color": color})
        N.draw()
